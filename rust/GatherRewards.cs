...
{
    [Info("Gather Rewards", "CanopySheep", "1.4.4", ResourceId = 770)]
    ...
    class GatherRewards : RustPlugin
    {
        ...
        void OnDispenserGather(ResourceDispenser dispenser, BaseEntity entity, Item item)
        {
            ...
            if (player)
            {
                ...
                if (resource != null && amount != 0)
                {
                    if (amount > 0)
                    {
                        if (config.Settings.UseEconomics && Economy) { Economy.CallHook("Deposit", player.userID, amount); }
                    ...
                    else
                    {
                        ...
                        if (config.Settings.UseEconomics && Economy) { Economy.CallHook("Withdraw", player.userID, amount); }
        ...
        void OnEntityDeath(BaseCombatEntity entity, HitInfo info)
        {
            ...
            if (amount != 0)
            {
                if (amount > 0)
                {
                    if (config.Settings.UseEconomics && Economy)
                    {
                        Economy.CallHook("Deposit", player.userID, amount);
                    }
                ...
                else
                {
                    ...
                    if (config.Settings.UseEconomics && Economy)
                    {
                        Economy.CallHook("Withdraw", player.userID, amount);
                    }
                    if (config.Settings.UseServerRewards && ServerRewards)
                    {
