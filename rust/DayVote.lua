...
function PLUGIN:SetDayTime(NewTime)
	if not AllowAirdrop then AllowAirdrop = true end
	if NewTime == nil then NewTime = self.Config.Settings.DayHour end
	VSky.Cycle.Hour = tonumber(NewTime)
end
...
function PLUGIN:OnTick()
	...
	if self.Config.Vote.Enabled == "true" then
		if not CanVote then
			if tostring(VSky.Cycle.Hour):match"([^.]*)" == self.Config.Settings.DayHour:match"([^.]*)" then
				CanVote = true
			end
			else
			if self.Config.Settings.AutoSkip == "true" then
				if tostring(VSky.Cycle.Hour):match"([^.]*)" == self.Config.Settings.NightHour:match"([^.]*)" then
...
function PLUGIN:cmdDayVote(player, cmd, args)
	...
	if args.Length == 0 then
		...
		if func == "set" then
			...
			if not tonumber(NewTime) or tonumber(NewTime) < 0 or tonumber(NewTime) > 23.9 then
				self:RustMessage(player, self:Lang(player, "NotNumber"))
				return
			end
			if self.Config.Settings.ShowTimeChanges == "true" then
				local message = FormatMessage(self:Lang(player, "AdminTimeSet"), { set = NewTime, current = tostring(VSky.Cycle.Hour):match"([^.]*)" })
				self:RustBroadcast(message)
				else
				local message = FormatMessage(self:Lang(player, "TimeSet"), { set = NewTime, current = tostring(VSky.Cycle.Hour):match"([^.]*)" })
				self:RustMessage(player, message)
			end
			if tonumber(NewTime) == tonumber(self.Config.Settings.NightHour) then
				CanVote = false
		...
		if cmd == "time" then
			if arg.Args then
				if arg.Args.Length > 0 then
					local NewTime = arg.Args[0]
					if not tonumber(NewTime) or tonumber(NewTime) < 0 or tonumber(NewTime) > 23.9 then NewTime = 0 end
					if self.Config.Settings.ShowTimeChanges == "true" then
						local message = FormatMessage(self:Lang(nil, "AdminTimeSet"), { set = NewTime, current = tostring(VSky.Cycle.Hour):match"([^.]*)" })
						self:RustBroadcast(message)
						else
						local message = FormatMessage(self:Lang(nil, "TimeSet"), { set = NewTime, current = tostring(VSky.Cycle.Hour):match"([^.]*)" })
						if arg.connection then
							local player = arg.connection.player
							self:Console(message, player, 1)
							else
							self:Console(message, nil, 2)
						end
					end
					if tonumber(NewTime) == tonumber(self.Config.Settings.NightHour) then
						CanVote = false
						if arg.connection then
							local player = arg.connection.player
							self:Console(self:Lang(nil, "SetToNightHour"), player, 1)
							else
							self:Console(self:Lang(nil, "SetToNightHour"), nil, 2)
						end
						else
						CanVote = true
					end
					if ActiveVote then
						ActiveVote = false
						self:DestroyTimers()
						self:RustBroadcast(self:Lang(nil, "SetVoteClose"))
						else
						if RevoteWait then
							self:DestroyTimers()
							self:RustMessage(player, self:Lang(nil, "SetRevoteClose"))
						end
					end
					return
				end
...
function PLUGIN:newVote(call, creq)
	...
	if call == "auto" then
		if string.sub(self.Config.Settings.AllowAutoNights, 1, 1) == "+" then
			...
			if AutoCurrentNight >= AllowAutoNights + 1 then
				AutoCurrentNight = 0
				local message = FormatMessage(self:Lang(nil, "SkipPlusAutoSkip"), { allowed = AllowAutoNights + 1 })
				self:RustBroadcast(message)
				return
			end
			else
			if self.Config.Settings.AllowAutoNights ~= "1" then
				...
				if AutoCurrentNight > 1 then
					local message = FormatMessage(self:Lang(nil, "SkipAutoSkip"), { allowed = self.Config.Settings.AllowAutoNights, current = AutoCurrentNight })
					self:RustBroadcast(message)
					return
				end
			end
		end
		self:RustBroadcast(self:Lang(nil, "AutoSkip"))
		self:SetDayTime()
		return
	end
	local pCount = self:getPlayerCount()
	...
	if call == "cycle" then
		if string.sub(self.Config.Settings.AllowVoteNights, 1, 1) == "+" then
			...
			if CurrentNight >= AllowVoteNights + 1 then
				CurrentNight = 0
				local message = FormatMessage(self:Lang(nil, "PlusVoteSkip"), { allowed = AllowVoteNights + 1 })
				self:RustBroadcast(message)
				return
			end
			else
			if self.Config.Settings.AllowVoteNights ~= "1" then
				...
				if CurrentNight > 1 then
					local message = FormatMessage(self:Lang(nil, "VoteSkip"), { allowed = self.Config.Settings.AllowVoteNights, current = CurrentNight })
					self:RustBroadcast(message)
					return
				end
			end
	...
	if call == "cycle" then
		...
		if isPercent then
			local pCount = self:getPlayerCount()
			message = FormatMessage(self:Lang(nil, "PercentVoteOpen"), { required = VoteRequired, players = pCount, seconds = self.Config.Vote.VoteDuration })
			else
			message = FormatMessage(self:Lang(nil, "VoteOpen"), { required = VoteRequired, seconds = self.Config.Vote.VoteDuration })
		end
		self:RustBroadcast(message)
	end
	if call == "admin" then
		...
		if isPercent then
			local pCount = self:getPlayerCount()
			message = FormatMessage(self:Lang(nil, "ManualPercentVoteOpen"), { required = VoteRequired, players = pCount, seconds = self.Config.Vote.VoteDuration })
			else
			message = FormatMessage(self:Lang(nil, "ManualVoteOpen"), { required = VoteRequired, seconds = self.Config.Vote.VoteDuration })
		end
		self:RustBroadcast(message)
	end
	if self.Config.Vote.ShowProgress == "true" then
		...
		VoteProgress = timer.Repeat(tonumber(self.Config.Vote.ProgressInterval), 0, function()
			...
			if isPercent then
				local pCount = self:getPlayerCount()
				local yes, no = self:GetPercentCount(pCount)
				message = FormatMessage(self:Lang(nil, "PercentProgress"), { yes = yes, no = no, players = pCount, required = VoteRequired, seconds = VoteDuration })
				else
				message = FormatMessage(self:Lang(nil, "Progress"), { yes = VoteType["yes"], no = VoteType["no"], required = VoteRequired, seconds = VoteDuration })
			end
	...
	VoteEnd = timer.Once(tonumber(self.Config.Vote.VoteDuration), function()
		...
		if isPercent then
			...
			if tonumber(VoteType["yes"]) ~= 0 and tonumber(VoteType["yes"]) >= tonumber(Percent) then
				local yes, no = self:GetPercentCount(pCount)
				local message = FormatMessage(self:Lang(nil, "PercentVotePassed"), { required = VoteRequired, no = no, players = pCount })
				self:RustBroadcast(message)
				self:SetDayTime()
				return
			end
			if VoteType["yes"] == "0" and VoteType["no"] == "0" then
				self:RustBroadcast(self:Lang(nil, "VoteFailedNoVote"))
				else
				local yes, no = self:GetPercentCount(pCount)
				local message = FormatMessage(self:Lang(nil, "PercentVoteFailed"), { yes = yes, no = no, players = pCount, required = VoteRequired })
				self:RustBroadcast(message)
			end
			if self.Config.Revote.Enabled == "true" and AllowRevote then
				if self.Config.Revote.Announce == "true" then
					local message = FormatMessage(self:Lang(nil, "RevoteAlert"), { seconds = self.Config.Revote.WaitDuration })
					self:RustBroadcast(self:Lang(nil, "RevotePrefix").." "..message)
				end
			...
			if tonumber(VoteType["yes"]) > tonumber(VoteType["no"]) then
				...
				if CheckVote >= VoteRequired then
					local message = FormatMessage(self:Lang(nil, "VotePassed"), { yes = VoteType["yes"], no = VoteType["no"] })
					self:RustBroadcast(message)
					self:SetDayTime()
					return
				end
			...
			if VoteType["yes"] == "0" and VoteType["no"] == "0" then
				...
				local message = FormatMessage(self:Lang(nil, "VoteFailed"), { yes = VoteType["yes"], no = VoteType["no"], required = VoteRequired })
				self:RustBroadcast(message)
			end
			if self.Config.Revote.Enabled == "true" and AllowRevote then
				if self.Config.Revote.Announce == "true" then
...
function PLUGIN:reVote()
	...
	if isPercentRV then
		local pCount = self:getPlayerCount()
		message = FormatMessage(self:Lang(nil, "PercentVoteOpen"), { required = string.sub(self.Config.Revote.VoteRequired, 1, -2), players = pCount, seconds = self.Config.Revote.VoteDuration })
		else
		message = FormatMessage(self:Lang(nil, "VoteOpen"), { required = self.Config.Revote.VoteRequired, seconds = self.Config.Revote.VoteDuration })
	end
	self:RustBroadcast(self:Lang(nil, "RevotePrefix").." "..message)
	if self.Config.Revote.ShowProgress == "true" then
		...
		RevoteProgress = timer.Repeat(tonumber(self.Config.Revote.ProgressInterval), 0, function()
			...
			if isPercentRV then
				local pCount = self:getPlayerCount()
				local yes, no = self:GetPercentCount(pCount)
				message = FormatMessage(self:Lang(nil, "PercentProgress"), { yes = yes, no = no, players = pCount, required = string.sub(self.Config.Revote.VoteRequired, 1, -2), seconds = VoteDuration })
				else
				message = FormatMessage(self:Lang(nil, "Progress"), { yes = VoteType["yes"], no = VoteType["no"], required = self.Config.Revote.VoteRequired, seconds = VoteDuration })
			end
			if VoteDuration > 0 then self:RustBroadcast(self:Lang(nil, "RevotePrefix").." "..message) end
		end, self.Plugin)
	end
	RevoteEnd = timer.Once(tonumber(self.Config.Revote.VoteDuration), function()
		...
		if isPercentRV then
			...
			if tonumber(VoteType["yes"]) ~= 0 and tonumber(VoteType["yes"]) >= tonumber(Percent) then
				local yes, no = self:GetPercentCount(pCount)
				local message = FormatMessage(self:Lang(nil, "PercentVotePassed"), { required = string.sub(self.Config.Revote.VoteRequired, 1, -2), no = no, players = pCount })
				self:RustBroadcast(self:Lang(nil, "RevotePrefix").." "..message)
				self:SetDayTime()
				return
			end
			if VoteType["yes"] == "0" and VoteType["no"] == "0" then
				self:RustBroadcast(self:Lang(nil, "RevotePrefix").." "..self:Lang(nil, "VoteFailedNoVote"))
				else
				local yes, no = self:GetPercentCount(pCount)
				local message = FormatMessage(self:Lang(nil, "PercentVoteFailed"), { yes = yes, no = no, players = pCount, required = string.sub(self.Config.Revote.VoteRequired, 1, -2) })
				self:RustBroadcast(self:Lang(nil, "RevotePrefix").." "..message)
			end
			return
			else
			...
			if VoteType["yes"] == "0" and VoteType["no"] == "0" then
				...
				local message = FormatMessage(self:Lang(nil, "VoteFailed"), { yes = VoteType["yes"], no = VoteType["no"], required = self.Config.Revote.VoteRequired })
				self:RustBroadcast(self:Lang(nil, "RevotePrefix").." "..message)
			end
			return
		end
...
function PLUGIN:Console(message, player, call)
	if call == 1 then
		player:SendConsoleCommand("echo "..self:Lang(player, "Prefix")..message)
		else
		UnityEngine.Debug.LogWarning.methodarray[0]:Invoke(nil, util.TableToArray({ StripMessage(self:Lang(nil, "Prefix")..message) }))
	end
end
