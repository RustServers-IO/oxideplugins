...
class rankme:
    ...
    def __init__(self):
        ...
        self.Title = 'Rank-ME'
        self.Version = V(2, 6, 4)
        self.Author = 'SkinN - community upated'
    ...
    def OnEntityBuilt(self, planner, component):
        ...
        if planner and planner.GetOwnerPlayer():

            uid = self.playerid(planner.GetOwnerPlayer())

            if uid in self.db and self._IsPlayerInZone(planner.GetOwnerPlayer()):
    ...
    def OnEntityDeath(self, ent, info):
        ...
        if 'patrolhelicopter' in str(ent) and 'gibs' not in str(ent) and str(ent) in self.helis_cache:
    ...
    def OnEntityTakeDamage(self, ent, info):
        ...
        # Is victim an Helicopter?
        if 'patrolhelicopter' in str(ent) and 'gibs' not in str(ent):
