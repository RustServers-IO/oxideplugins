	...
	if args.Length == 0 then
		...
		if func == "toggle" then
			...
			if self.Config.Player.Enabled == "false" and self.Config.Clan.Enabled == "false" then
				...
				if LocPosTimer == nil or not LocPosTimer then self:LocPosTimer() end
			end
			self:SaveConfig()
...
function PLUGIN:OpenPlayerBank(player, target)
	...
	LocPos[player] = player.transform.position
...
function PLUGIN:LocPosTimer()
	if LocPosTimer then LocPosTimer:Destroy() end
	LocPosTimer = timer.Repeat(1, 0, function()
		for key, value in pairs(LocPos) do
			if UnityEngine.Vector3.Distance(key.transform.position, value) >= 1 then
				rust.ForcePlayerPosition(key, value.x, value.y, value.z)
			end
		end
	end, self.Plugin)
end
